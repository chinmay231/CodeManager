...
def _should_skip_file(p: Path, exclude_file_regexes: List[re.Pattern], max_bytes: int) -> bool:
    s = str(p)
...
def compile_codefile(
    root: Path,
    out_path: Path,
    include_exts: List[str],
    exclude_dirs: List[str],
    exclude_file_regexes: List[re.Pattern],
    max_bytes: int,
    order_mode: str = "path",
) -> None:
    root = Path(root)
    ...
    lines: List[str] = []
    for p in files:
        if _should_skip_file(p, exclude_file_regexes, max_bytes):
            continue
